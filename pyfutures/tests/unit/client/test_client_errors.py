...
async def test_handle_exception_in_callback(mocker, event_loop):
    ...
    mock_server.queue_response(
        req=b"\x00\x00\x00\x0517\x001\x00",
        res=b"\x00\x00\x00\x0f15\x001\x00DU1234567\x00",
    )
    ...
    def mock_managed_accounts(self, *args, **kwargs):
        return InteractiveBrokersClient.managedAccounts(
            client, accountsList="DU1234567,"
        )
