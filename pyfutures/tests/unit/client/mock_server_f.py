...
class MockServer:
    ...
    def __init__(self):
        ...
        self.reader.read.side_effect = self._read_generator

        self.respond = asyncio.Event()

        self.writer = MagicMock()
        self.writer.drain = AsyncMock()
        self.writer.write.side_effect = self._write_generator
    ...
    def _write_generator(self, msg):
        ...
        assert msg == self._requests.popleft(), "expected write value != actual write value"
    ...
    def queue_handshake(self):
        ...
        self.queue_transaction(
            ...
            responses=[b"\x00\x00\x00*176\x0020240303 20:39:51 Greenwich Mean Time\x00"],
        )

        # startApi message
        self.queue_transaction(
            request=b"\x00\x00\x00\t71\x002\x001\x00\x00",
