...
class InteractiveBrokersClient:
    ...
    async def request_front_contract_details(
        self, contract: IBContract
    ...
    async def _request_bars(
        ...
        self._log.info(
            f"{start_time} | {end_time} | {bar_size} | {duration} | {what_to_show} | {contract}"
        )
    ...
    async def request_whatif(self, detail: IBContractDetails) -> dict:
        ...
        return dict(
            ...
            initMarginBefore=float(order_state.initMarginBefore),
            maintMarginBefore=float(order_state.maintMarginBefore),
            equityWithLoanBefore=float(order_state.equityWithLoanBefore),
            initMarginChange=float(order_state.initMarginChange),
            maintMarginChange=float(order_state.maintMarginChange),
            equityWithLoanChange=float(order_state.equityWithLoanChange),
            initMarginAfter=float(order_state.initMarginAfter),
            maintMarginAfter=float(order_state.maintMarginAfter),
            equityWithLoanAfter=float(order_state.equityWithLoanAfter),
    ...
    def commissionReport(
        self, commissionReport: IBCommissionReport
    ):  # : Override the EWrapper
        """
    ...
    def accountSummary(
        ...
        if request.data.get("account") is not None:
            ...
            )  # multiple accounts not support yet
    ...
    def headTimestamp(self, reqId: int, headTimestamp: str):
        ...
        head_timestamp: pd.Timestamp = pd.to_datetime(
            headTimestamp, format="%Y%m%d-%H:%M:%S", utc=True
        )
    ...
    def subscribe_quote_ticks(
        ...
        cancel = functools.partial(
            ...
            cancel_func=functools.partial(
                self._eclient.cancelTickByTickData, reqId=request_id
            ),
        )
    ...
    def _subscribe_realtime_bars(
        ...
        cancel = functools.partial(
            ...
            request_id=request_id,
            cancel_func=functools.partial(
                self._eclient.cancelRealTimeBars, reqId=request_id
            ),
        )
    ...
    def _subscribe_historical_bars(
        ...
        cancel = functools.partial(
            ...
            request_id=request_id,
            cancel_func=functools.partial(
                self._eclient.cancelHistoricalData, reqId=request_id
            ),
        )
    ...
    async def request_historical_schedule(
        self, contract: IBContract, durationStr: str | None = None
